...
let
  ...
  selected =
    ...
    lib.genAttrs names (
      name:
      if lib.hasAttr name definitions then
        definitions.${name}
      else
        throw "evak.opencode-agents: unknown shared definition `${name}`"
    );
  ...
  mcpConfig = server: {
    type = server.type;
    command = if server.type == "local" then server.command ++ server.args else null;
    url = server.url;
  ...
  config = lib.mkIf cfg.enable {
    ...
    ++ lib.flatten (
      lib.mapAttrsToList (name: role: [
        {
          assertion =
            role.authentication.mode == "shared"
