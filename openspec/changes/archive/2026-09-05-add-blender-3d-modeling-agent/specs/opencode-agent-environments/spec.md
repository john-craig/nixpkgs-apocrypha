...
The system SHALL provide a stable `blender-3d-modeler` environment with an explicit local
Blender MCP declaration, project-edit permissions, and approval-gated or unavailable
arbitrary-code, destructive, render, export, network, device, and external-write operations.
...
#### Scenario: Blender modeler environment is selected
- **WHEN** a configured user selects the `blender-3d-modeler` environment
- **THEN** the generated environment SHALL contain only its declared Blender skills and MCP servers, preserve credential and project boundaries, and keep high-risk Blender operations approval-gated or unavailable
...
The repository SHALL document the mapping of all 21 source environments and identify any Codex behavior that has no direct OpenCode equivalent.
